...
putter :: Palo
putter (Habilidad _ precision) = UnTiro {
  velocidad = 10,
...
madera :: Palo
madera (Habilidad _ precision) = UnTiro {
  velocidad = 100,
...
hierro :: Int -> Palo
hierro n (Habilidad fuerza precision) = UnTiro {
  velocidad = fuerza * n,
  precision = precision `div` n,
  altura = max (n-3) 0
}
...
tunel:: Tiro -> Tiro
tunel (UnTiro velocidad precision _) | precision > 90 = UnTiro (velocidad*2) 100 0
                                          |otherwise = UnTiro 0 0 0

laguna:: Tiro -> Double-> Tiro
laguna (UnTiro velocidad precision altura) largoLaguna| velocidad > 80 && between 1 5 altura = UnTiro velocidad precision (div altura largoLaguna)
                                          |otherwise =  UnTiro 0 0 0

hoyo:: Tiro -> Tiro
hoyo (UnTiro velocidad precision altura) | between 1 5 velocidad = UnTiro (velocidad*2) 100 0
                                          |otherwise =  UnTiro 0 0 0

pasaObs:: (Tiro -> Tiro) -> Tiro -> Bool
pasaObs obs (UnTiro velocidad precision altura) = obs (UnTiro velocidad precision altura) /= UnTiro 0 0 0

paloMasUtil :: Jugador -> (Tiro -> Tiro) -> Palo
paloMasUtil jugador obstaculos = filter (\palo -> pasaObs obstaculos (golpe jugador palo) ) palos

obstaculosSuperados :: Tiro -> (Tiro -> Tiro) -> Int
obstaculosSuperados tiro = length . takeWhile (\obs -> pasaObs obs tiro)

paloMasUtil :: Jugador -> (Tiro -> Tiro) -> Palo
paloMasUtil jugador obstaculos = maximoSegun (\palo -> obstaculosSuperados (golpe jugador palo) obstaculos) palos